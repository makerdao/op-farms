...
//
// DssVestMock - Mock of DssVest, a token vesting contract
//
...
abstract contract DssVestMock {
    ...
    */
    function create(address _usr, uint256 _tot, uint256 _bgn, uint256 _tau, uint256 _eta, address _mgr) external lock auth returns (uint256 id) {
        ...
        require(_bgn < block.timestamp + TWENTY_YEARS, "DssVest/bgn-too-far");
        require(_bgn > block.timestamp - TWENTY_YEARS, "DssVest/bgn-too-long-ago");
        ...
        awards[id] = Award({
            usr: _usr,
            bgn: toUint48(_bgn),
            clf: toUint48(_bgn + _eta),
            fin: toUint48(_bgn + _tau),
            tot: toUint128(_tot),
            rxd: 0,
            mgr: _mgr,
            res: 0
    ...
    */
    function _vest(uint256 _id, uint256 _maxAmt) internal lock {
        ...
        amt = min(amt, _maxAmt);
        awards[_id].rxd = toUint128(_award.rxd + amt);
        pay(_award.usr, amt);
        emit Vest(_id, amt);
    }
    ...
    */
    function accrued(uint256 _time, uint48 _bgn, uint48 _fin, uint128 _tot) internal pure returns (uint256 amt) {
        if (_time < _bgn) {
            ...
            amt = (_tot * (_time - _bgn)) / (_fin - _bgn); // 0 <= amt < _award.tot
        }
    }
    ...
    */
    function unpaid(uint256 _time, uint48 _bgn, uint48 _clf, uint48 _fin, uint128 _tot, uint128 _rxd) internal pure returns (uint256 amt) {
        amt = _time < _clf ? 0 : accrued(_time, _bgn, _fin, _tot) - _rxd;
    }
        ...
        if (_end < _award.fin) {
            ...
            if (end < _award.bgn) {
                ...
                awards[_id].tot = toUint128(
                    unpaid(_end, _award.bgn, _award.clf, _award.fin, _award.tot, _award.rxd) + _award.rxd
                );
            }
        }
...
contract DssVestMintableMock is DssVestMock {
    ...
    */
    constructor(address _gem) DssVestMock() {
...
contract DssVestSuckableMock is DssVestMock {
    ...
    */
    constructor(address _chainlog) DssVestMock() {
    ...
    */
    function pay(address _guy, uint256 _amt) override internal {
        ...
        vat.suck(chainlog.getAddress("MCD_VOW"), address(this), _amt * RAY);
...
/*
    Transferrable token DssVest. Can be used to enable streaming payments of
     ...
     contributors.
*/
contract DssVestTransferrableMock is DssVestMock {
    ...
    */
    constructor(address _czar, address _gem) DssVestMock() {
